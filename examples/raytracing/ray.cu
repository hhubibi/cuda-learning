...
__global__ void kernel(uchar3* ptr, int width, int height) {
...
void init_const_sphere() {
    ...
    printf("init const sphere success\n");
}

void destroy_const_sphere() {
    cudaFree(s);
    printf("destroy const sphere success\n");
}
...
void draw(uchar3* ptr, int width, int height) {
    ...
    kernel<<<grid_dim, block_dim>>>(ptr, width, height);
}
